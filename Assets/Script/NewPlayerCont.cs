...
public class New : MonoBehaviour
{
    ...
    private bool CanMove(Vector2 direction)
    {
        ...
        // Проверяем, если есть столкновение
        Collider2D hitCollider = Physics2D.OverlapPoint(newPosition);

        // Если персонаж столкнулся с объектом "BlockBoxes", мы просто возвращаем true,
        // чтобы он мог пройти через него
        if (hitCollider != null && hitCollider.CompareTag("BlockBox"))
        {
            return true;
        }

        // Проверяем только ящики, если есть столкновение
        if (hitCollider != null && hitCollider.CompareTag("Pushable"))
        {
            // Проверяем, является ли объект с которым сталкиваемся ящиком
            GameObject box = hitCollider.gameObject;
            Vector3 boxNewPosition = box.transform.position + (Vector3)direction;
            // Проверяем, если ящик не может двигаться, то и персонаж не может двигаться
            if (!CanMoveBox(box, boxNewPosition, direction))
            {
                //Debug.Log("Cannot move. Box cannot move.");
                return false;
            }
        }
        ...
        {
            //Debug.Log("Cannot move to the target position. Collision detected.");
            return false;
        }
    ...
    private bool CanMoveBox(GameObject box, Vector3 boxNewPosition, Vector2 direction)
    {
        ...
        // Проверяем, если столкнулись с объектом, который блокирует ящики
        Collider2D hitCollider = Physics2D.OverlapPoint(boxNewPosition);
        if (hitCollider != null && hitCollider.CompareTag("BlockBox"))
        {
            //Debug.Log("Cannot move box. BlockBoxes detected.");
            return false;
        }
